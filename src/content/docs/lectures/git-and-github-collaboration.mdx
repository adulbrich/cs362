...
Here's an overview of the workflow. We'll dig deeper into each step in the following sections.
...
- Integrating changes from one branch into another while maintaining a linear history (which might be preferred by some teams over merging)
...
Add background color to the document. Apply the style to the `<body>` tag. Use color [#99ccff](https://www.colorhexa.com/99ccff).
...
Similar to issues, the Projects tab can be toggled on or off in the repository Settings.
...
A tag is thus associated with a specific commit, and you can use it to refer to that commit in the future. For example, if you want to check out the code from version 1.0, you can do:
...
- [Version Control and Branch Management](https://abseil.io/resources/swe-book/html/ch16.html) in Software Engineering at Google
- [Code Review](https://abseil.io/resources/swe-book/html/ch09.html) in Software Engineering at Google
