...
By default, spies created with `vi.spyOn()` continue to call the original function they're spying on. This means our previous tests were still executing the real `Database.save()` method.
...
- t exactly 12:00 AM, it displays "midnight"
...
Note that we could setup test fixtures such as `beforeEach()` and `afterEach()` to avoid repeating the `vi.useFakeTimers()` and `vi.useRealTimers()` calls in each test. This would help keep our tests clean and focused on the specific behavior being tested.
...
MSW intercepts network requests at a higher level, creating a fake server that responds to requests regardless of which HTTP client makes them. This approach:
...
```js
// registerUser.test.js
import { Database as FakeDatabase } from "./inMemoryDatabase.js";
