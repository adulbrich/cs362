...
Line coverage (sometimes called statement coverage, even though they are not an exact match) is the most basic and commonly used metric, reporting the percentage of code lines executed during testing. While intuitive, it has limitations—a line with complex logic might be "covered" even if only one code path through it is tested.
...
graph TD
  ...
  linkStyle 2,3,5,9 stroke:#16a34a

  classDef covered fill:#9f9,stroke:#484,stroke-width:2px,color:#333
  classDef uncovered fill:#f99,stroke:#844,stroke-width:2px,color:#333
  classDef partial fill:#ff9,stroke:#884,stroke-width:2px,color:#333

  class B,D,E,G covered
  class C,F,H uncovered
```

We outline one of the possible paths (in green bold) which will be executed if `isAdmin` is false, `isAuthor` is true, and `isPublished` is true.
...
A common property-based testing library for JavaScript is [fast-check](https://fast-check.dev/) which integrates with [Vitest](https://www.npmjs.com/package/@fast-check/vitest). It allows you to define properties and automatically generates test cases to verify them. It also supports [fuzzing](https://fast-check.dev/docs/advanced/fuzzing/).
