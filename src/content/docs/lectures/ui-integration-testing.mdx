...
{/* IMPROVE: Add a note about why we are using type="module" and the benefits of ESM. */}
...
{/* FIX: Ensure the import paths and package names are correct and up-to-date. */}
...
While it may seem that we're testing implementation with the [`toHaveAttribute()`](https://vitest.dev/guide/browser/assertion-api.html#tohaveattribute) matcher, we're actually verifying the expected behavior of the application. The test checks that the photo card is created with the correct attributes and is visible to the user.
...
The second one does not rely on knowing that we have a list that encapsulates the photo cards. It simply checks that the image is not present in the DOM.
