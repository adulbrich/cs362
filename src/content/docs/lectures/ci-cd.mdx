...
In the example below, we run our application against the live version of our application. This is a simple example, but you can customize it to suit your needs.
...
Now every time you commit, Lefthook will run the specified tasks. If any of the tasks fail, the commit will be aborted, allowing you to fix the issues before pushing your code. Same applies for the pre-push hook.
...
> A promising alternative to both ESLint and Prettier is [Biome](https://biomejs.dev/), which combines linting, formatting, and type checking into a single tool. It is designed to be fast and efficient, making it a great choice for modern JavaScript and TypeScript projects.
...
The crucial point is that you want **the whole team to agree on a single set of rules for both linting and formatting**. If you don't, you might end up with many diffs in your pull requests that are just formatting changes, making it harder to review the actual code changes.

Most of the time, your linter and formatter options will be configured in your repository and shared among all developers. 
