...
You can run the test by clicking the "Run" button next to the test name. 
...
You might remember from our UI-based integration testing lecture notes that we used Testing Library to mimic user interactions. Playwright provides a similar API, called [locators](https://playwright.dev/docs/locators) to find elements on a page.
...
We expect to also create a todo by hitting the `Enter` key. This is a common pattern in many applications, and it improves the user experience by allowing users to quickly add todos without having to click the "Add" button.
...
This test captures the `todoItem` once it has been created. Playwright allows us to **chain locators** together, so we can use the `todoItem` locator to find the checkbox and click it. After that, we check if the checkbox is checked and if the todo item has a line-through style applied to it.
...
Playwright's test fixtures allow us to create "named" fixtures that can be reused across different tests.
...
Then we can write a test to check for accessibility issues on our application. 
...
Let's test our online textbook homepage:
...
An important consideration in the modern web is to test your application on mobile devices. While Playwright provides a huge list of supported devices, let's start simple:
