...
:::note[Blue Brain Nexus Benchmarks]
Here's [an example of benchmarks for a data management platform](https://bluebrainnexus.io/v1.8.x/docs/delta/benchmarks/v1.4.2.html) using [Gatling](https://gatling.io/). Note the intersection of performance testing with load testing, as it establishes the baseline performance under normal conditions before simulating higher loads (discussed next).
:::
...
{/* IMPROVE: Add a diagram illustrating the circuit breaker pattern (Closed, Open, Half-Open states). */}
...
- **Uber**: [Uber's safe approach to mobile network API migration](https://blog.droidchef.dev/shadow-calls-and-circuit-breakers-ubers-safe-approach-to-mobile-network-api-migration/) using shadow calls and circuit breakers to monitor issues
{/* - **Amazon**: ... */}
...
{/* FIX: This link seems to point to a generic blog post. Verify if it's the intended resource for Playwright synthetic monitoring. */}
