...
Vitest provides a `beforeEach()` function that allows you to set up a fixture before each test. This is useful for creating a common setup for multiple tests.
...
Here we are using `beforeEach()` to create a new instance of the `Calculator` class before each test. This reduces duplication and makes the tests easier to read and understand. 

`afterEach()` is a similar function that runs after each test. This can be useful for cleaning up any resources or state that were created during the test. 

We could also use `beforeAll()`/`afterAll()` to create/delete the fixture once for all tests in the group. This is useful when the fixture is expensive to create and doesn't change between tests. 
