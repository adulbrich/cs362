...
In this assignment, you will utilize tools explored in class to implement integration tests for small UI-based applications. You will write tests for two distinct applications, with specific testing goals for each, as detailed below.

Before you begin writing tests, set up your testing environment. Vitest is already included as a dependency in `package.json`. Install it by running:
...
* [The jsdom or happy-dom Vitest test environment](https://vitest.dev/guide/environment.html) to provide a DOM-based environment for testing the apps.
* [The DOM Testing Library](https://testing-library.com/docs/dom-testing-library/intro) for locating UI elements as a user would.
* [The User Event library](https://testing-library.com/docs/user-event/intro) for simulating user interactions with the apps.
{/* * [The Happy DOM environment](https://github.com/capricorn86/happy-dom) as an alternative DOM implementation for testing (can be used instead of jsdom). */}
